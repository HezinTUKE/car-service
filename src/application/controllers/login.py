...
from application.schemas.auth_request_schema import (
    AuthRequestSchema,
    ConfirmUserRequestSchema,
...
class LoginController:
    ...
    async def signup(request: AuthRequestSchema):
    ...
    async def confirm_email(request: ConfirmUserRequestSchema):
    ...
    async def reset_password(request: ResetPasswordRequestSchema):
        return await LoginHandler.reset_password(
            email=str(request.email), confirmation_code=request.confirmation_code, new_password=request.new_password
        )
    ...
    async def get_current_user_info(
        ...
        return ProfileResponseSchema(
            user_id=current_user.user_id, email=current_user.email, permissions=current_user.groups
        )
