...
from sqlalchemy import UUID, String, Boolean, Integer, DateTime
...
class UserSetupModel(Base):
    ...
    updated_at: Mapped[int] = mapped_column(
        Integer, nullable=False, default=int(time.time()), onupdate=int(time.time())
    )

    user_car_relation: Mapped[list["UserCarRelationModel"]] = relationship(
        "UserCarRelationModel", back_populates="user_setup", lazy="joined"
    )
